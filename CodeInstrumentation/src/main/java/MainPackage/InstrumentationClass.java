...
import InnerData.InstrumentationData;
...
import static MainPackage.Main.HANDLER_NAME;
import static MainPackage.Main.LOGGER_NAME;
...
public class InstrumentationClass {
    ...
    private CompilationUnit instrument(List<MethodDeclaration> methods, Items item, List<InnerData.Locations> locations){
    ...
    private void configLogger(List<MethodDeclaration> methods){
        for (MethodDeclaration method : methods) {
            ...
            if (block.isPresent()){
                ...
                statements.add(0, JavaParser.parseStatement(HANDLER_NAME + ".setLevel(Level.ALL);"));
                statements.add(1, JavaParser.parseStatement(LOGGER_NAME + ".addHandler(" + HANDLER_NAME + ");"));
                statements.add(2, JavaParser.parseStatement(LOGGER_NAME + ".setLevel(Level.ALL);"));
            }
        }
    }
